...
class LoadTestUser {
	...
	async connectWebSocket() {
		return new Promise((resolve, reject) => {
			...
			const wsUrl = sessionId
				? `${CONFIG.WS_URL}?session=${sessionId}`
				: CONFIG.WS_URL;
	...
	handleMessage(message) {
		...
		if (message.type !== "pong") {
			console.log(
				`🔔 User ${this.userId} received WebSocket message:`,
		...
		switch (message.type) {
			case "card_created":
				if (
					...
					console.log(
						`⚠️ User ${this.userId} received card_created message without card ID:`,
						message,
					);
				}
				break;
			...
			case "card_moved":
				if (
					...
					console.log(
						`⚠️ User ${this.userId} received card_moved message without card ID:`,
						message,
					);
				}
				break;
...
class LoadTester {
	...
	logDebugInfo() {
		...
		console.log(
			`Active WebSocket connections: ${this.users.filter((u) => u.ws && u.ws.readyState === 1).length}`,
		);
	}
