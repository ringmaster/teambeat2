...
import {
	getTestUser,
	setupMultiUserContexts,
	TestUsers,
} from "./fixtures/auth-helpers";

test.describe("Multi-User Collaboration", () => {
	test("should show real-time card creation across multiple users", async ({
		browser,
	}) => {
		...
		const [facilitatorPage, participant1Page, participant2Page] =
			await Promise.all(contexts.map((ctx) => ctx.newPage()));
		...
		await p1Column
			...
			.fill("Participant 1 card");
		await p1Column.locator('[data-testid="add-card-button"]').click();

		// Other users should see the card appear
		await expect(
			facilitatorPage.locator("text=Participant 1 card"),
		).toBeVisible({ timeout: 3000 });
		await expect(
			participant2Page.locator("text=Participant 1 card"),
		).toBeVisible({ timeout: 3000 });
		...
		await p2Column
			.locator('[data-testid="card-input"]')
			.fill("Participant 2 improvement");
		await p2Column.locator('[data-testid="add-card-button"]').click();

		// All users should see both cards
		await expect(
			facilitatorPage.locator("text=Participant 2 improvement"),
		).toBeVisible({ timeout: 3000 });
		await expect(
			participant1Page.locator("text=Participant 2 improvement"),
		).toBeVisible({ timeout: 3000 });

		// Verify card attribution
		const p1Card = facilitatorPage.locator(
			'[data-testid="card"]:has-text("Participant 1 card")',
		);
		await expect(p1Card.locator('[data-testid="card-author"]')).toContainText(
			"Participant One",
		);

		const p2Card = facilitatorPage.locator(
			'[data-testid="card"]:has-text("Participant 2 improvement")',
		);
		await expect(p2Card.locator('[data-testid="card-author"]')).toContainText(
			"Participant Two",
		);
	...
	test("should handle concurrent voting with real-time updates", async ({
		browser,
	}) => {
		const { contexts } = await setupMultiUserContexts(
			browser,
		...
		await expect(p1Card.locator('[data-testid="vote-count"]')).toContainText(
			"2",
		);
		await expect(p2Card.locator('[data-testid="vote-count"]')).toContainText(
			"2",
		);

		// Check remaining votes decreased for both participants
		await expect(
			participant1Page.locator('[data-testid="votes-remaining"]'),
		).toContainText("2 votes remaining");
		await expect(
			participant2Page.locator('[data-testid="votes-remaining"]'),
		).toContainText("2 votes remaining");
	...
	test("should synchronize presence indicators correctly", async ({
		browser,
	}) => {
		const { contexts } = await setupMultiUserContexts(
			browser,
		...
		await expect(
			participant1Page.locator('[data-testid="presence-list"]'),
		).toContainText("Test Facilitator");
		...
		await expect(
			facilitatorPage.locator('[data-testid="presence-list"]'),
		).toContainText("Participant Two", { timeout: 3000 });
		await expect(
			participant1Page.locator('[data-testid="presence-list"]'),
		).toContainText("Participant Two");
		await expect(
			participant2Page.locator('[data-testid="presence-list"]'),
		).toContainText("Test Facilitator");
		await expect(
			participant2Page.locator('[data-testid="presence-list"]'),
		...
		await expect(
			facilitatorPage.locator('[data-testid="presence-list"]'),
		).not.toContainText("Participant One", { timeout: 35000 });
		await expect(
			participant2Page.locator('[data-testid="presence-list"]'),
		).not.toContainText("Participant One", { timeout: 35000 });
	...
	test("should handle facilitator scene changes for all users", async ({
		browser,
	}) => {
		const { contexts } = await setupMultiUserContexts(
			browser,
		...
		await expect(
			facilitatorPage.locator('[data-testid="current-scene"]'),
		).toContainText("Brainstorm");
		await expect(
			participant1Page.locator('[data-testid="current-scene"]'),
		).toContainText("Brainstorm");
		await expect(
			participant2Page.locator('[data-testid="current-scene"]'),
		).toContainText("Brainstorm");

		// Participants should see card input fields
		await expect(
			participant1Page.locator('[data-testid="card-input"]'),
		).toBeVisible();
		await expect(
			participant2Page.locator('[data-testid="card-input"]'),
		).toBeVisible();
		...
		await expect(
			facilitatorPage.locator('[data-testid="current-scene"]'),
		).toContainText("Review", { timeout: 3000 });
		await expect(
			participant1Page.locator('[data-testid="current-scene"]'),
		).toContainText("Review", { timeout: 3000 });
		await expect(
		...
		await expect(
			participant1Page.locator('[data-testid="vote-button"]').first(),
		).toBeVisible();
		await expect(
			participant2Page.locator('[data-testid="vote-button"]').first(),
		).toBeVisible();
	...
	test("should handle timer synchronization across users", async ({
		browser,
	}) => {
		const { contexts } = await setupMultiUserContexts(
			browser,
		...
		await expect(
			facilitatorPage.locator('[data-testid="timer-controls"]'),
		).toBeVisible();
		await expect(
			participant1Page.locator('[data-testid="timer-controls"]'),
		).not.toBeVisible();
		await expect(
			participant2Page.locator('[data-testid="timer-controls"]'),
		).not.toBeVisible();
		...
		await expect(
			facilitatorPage.locator('[data-testid="timer-display"]'),
		).toBeVisible();
		await expect(
			participant1Page.locator('[data-testid="timer-display"]'),
		).toBeVisible({ timeout: 3000 });
		await expect(
			participant2Page.locator('[data-testid="timer-display"]'),
		).toBeVisible({ timeout: 3000 });
		...
		await expect(
			facilitatorPage.locator('[data-testid="timer-paused"]'),
		).toBeVisible();
		await expect(
			participant1Page.locator('[data-testid="timer-paused"]'),
		).toBeVisible({ timeout: 3000 });
		await expect(
			participant2Page.locator('[data-testid="timer-paused"]'),
		).toBeVisible({ timeout: 3000 });
	...
	test("should handle card grouping with real-time updates", async ({
		browser,
	}) => {
		const { contexts } = await setupMultiUserContexts(
			browser,
		...
		await expect(
			facilitatorPage.locator('[data-testid="card-group"]'),
		).toBeVisible();
		await expect(
			participant1Page.locator('[data-testid="card-group"]'),
		).toBeVisible({ timeout: 3000 });
		await expect(
			participant2Page.locator('[data-testid="card-group"]'),
		).toBeVisible({ timeout: 3000 });

		// Group should contain the grouped cards
		const facilitatorGroup = facilitatorPage
			.locator('[data-testid="card-group"]')
			.first();
		const participant1Group = participant1Page
			.locator('[data-testid="card-group"]')
			.first();
		const participant2Group = participant2Page
			.locator('[data-testid="card-group"]')
			.first();

		await expect(facilitatorGroup.locator('[data-testid="card"]')).toHaveCount(
			2,
		);
		await expect(participant1Group.locator('[data-testid="card"]')).toHaveCount(
			2,
		);
		await expect(participant2Group.locator('[data-testid="card"]')).toHaveCount(
			2,
		);
	...
	test("should handle rapid concurrent card creation", async ({ browser }) => {
		...
		await expect(participant1Page.locator('[data-testid="card"]')).toHaveCount(
			6,
			{ timeout: 5000 },
		);
		await expect(participant2Page.locator('[data-testid="card"]')).toHaveCount(
			6,
			{ timeout: 5000 },
		);
	...
	test("should handle user disconnection and reconnection", async ({
		...
		await expect(
			participant1Page.locator('[data-testid="offline-indicator"]'),
		...
		await expect(
			facilitatorPage.locator('[data-testid="presence-list"]'),
		).toContainText("Participant One", { timeout: 10000 });
		await expect(facilitatorPage.locator("text=Offline card")).toBeVisible({
			timeout: 10000,
		});
