...
import { getSceneFlags, setSceneFlags } from '../../../src/lib/server/repositories/scene';
...
describe('POST /api/boards/[id]/scenes', () => {
	...
	it('should fail when user is not facilitator', async () => {
		...
		expect(response.status).toBe(403);
		expect(data.success).toBe(false);
		expect(data.error).toBe('Only administrators and facilitators can create scenes');
	});
