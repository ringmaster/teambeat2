...
import { findUserById, updateUserPassword, deleteUserById } from '../../../src/lib/server/repositories/user';
...
describe('Password Reset Token Generation and Validation', () => {
	...
	describe('validatePasswordResetToken', () => {
		...
		it('should reject token after password change (auto-revocation)', async () => {
			...
			const mockUser = { id: userId, passwordHash: newPasswordHash, email: 'test@example.com' };
			vi.mocked(findUserById).mockResolvedValue(mockUser as any);
...
describe('POST /api/admin/users/generate-reset-token', () => {
	...
	it('should generate reset token for admin user', async () => {
		...
		// Mock admin session
		vi.mocked(getSessionFromCookie).mockResolvedValue({ userId: adminUserId } as any);

		// Mock admin user
		vi.mocked(db.select).mockReturnValue({
			from: vi.fn().mockReturnValue({
				where: vi.fn().mockReturnValue({
					limit: vi.fn().mockResolvedValue([{ id: adminUserId, isAdmin: true }])
				})
			})
		} as any);
