...
describe('POST /api/health-responses', () => {
	...
	it('should validate red/yellow/green question ratings', async () => {
		...
		expect(response.status).toBe(400);
		expect(data.success).toBe(false);
		expect(data.error).toBe('Red/Yellow/Green questions require rating of 1, 3, or 5');
	});
	...
	it('should fail when board is not in draft or active state', async () => {
		...
		expect(response.status).toBe(400);
		expect(data.success).toBe(false);
		expect(data.error).toBe('Responses can only be submitted when board is draft or active');
	});
});
