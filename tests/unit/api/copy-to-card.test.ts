...
describe('POST /api/agreements/[id]/copy-to-card', () => {
	...
	it('returns 403 when board is completed', async () => {
		...
		const mockBoard = {
			...
			columns: [{ id: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11', title: 'Column 1' }],
			hiddenColumnsByScene: {}
		};
	...
	it('returns 403 when user is not facilitator or admin', async () => {
		...
		expect(response.status).toBe(403);
		expect(data.success).toBe(false);
		expect(data.error).toBe('Only facilitators and admins can copy agreements to cards');
	});
...
describe('POST /api/comments/[id]/copy-to-card', () => {
	...
	it('creates card when scene allows adding cards', async () => {
		...
		const selectMock = vi.fn()
			.mockReturnValueOnce({ // First call for comment data
				from: vi.fn(() => ({
					innerJoin: vi.fn(() => ({
						innerJoin: vi.fn(() => ({
							where: vi.fn(() => ({
			...
			.mockReturnValueOnce({ // Second call for reactions
				from: vi.fn(() => ({
					where: vi.fn(() => ({
...
describe('POST /api/health-questions/[questionId]/copy-to-card', () => {
	...
	it('creates card when scene allows adding cards', async () => {
		...
		// Mock column query
		const selectMock = vi.fn()
			.mockReturnValueOnce({ // First call for question data
				from: vi.fn(() => ({
					innerJoin: vi.fn(() => ({
						innerJoin: vi.fn(() => ({
							where: vi.fn(() => ({
			...
			.mockReturnValueOnce({ // Second call for column
				from: vi.fn(() => ({
					where: vi.fn(() => ({
			...
			.mockReturnValueOnce({ // Third call for responses
				from: vi.fn(() => ({
