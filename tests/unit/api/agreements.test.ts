...
import {
	GET,
	POST,
} from "../../../src/routes/api/boards/[id]/agreements/+server";
...
describe("GET /api/boards/[id]/agreements", () => {
	...
	it("should return enriched agreements when user has access", async () => {
		...
		vi.mocked(buildEnrichedAgreementsData).mockResolvedValue(
			mockAgreements as any,
		);
		...
		expect(buildEnrichedAgreementsData).toHaveBeenCalledWith(
			"board-1",
			mockBoard,
		);
		expect(data).toEqual({
...
describe("POST /api/boards/[id]/agreements", () => {
	...
	it("should create agreement when facilitator posts valid data", async () => {
		...
		expect(broadcastAgreementsUpdated).toHaveBeenCalledWith(
			"board-1",
			mockAgreements,
		);
		expect(data).toEqual({
