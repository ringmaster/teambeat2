...
describe("POST /api/cards/[id]/vote", () => {
	...
	it("should successfully add a vote when all conditions are met", async () => {
		...
		expect(broadcastVoteChanged).toHaveBeenCalledWith(
			"board-1",
			"card-1",
			3,
			"user-1",
	...
	it("should successfully remove a vote when delta is -1", async () => {
		...
		vi.mocked(getVoteContext).mockResolvedValue(
			createMockVoteContext({ currentVoteCount: 1 }),
		);
		vi.mocked(castVote).mockResolvedValue({
			action: "removed",
			voteId: "vote-1",
	...
	it("should return 403 when user does not have access to series", async () => {
		vi.mocked(requireUserForApi).mockReturnValue(mockUser);
		vi.mocked(getVoteContext).mockResolvedValue(
			createMockVoteContext({ userRole: null }),
		);
	...
	it("should return 400 when trying to remove votes but user has none", async () => {
		vi.mocked(requireUserForApi).mockReturnValue(mockUser);
		vi.mocked(getVoteContext).mockResolvedValue(
			createMockVoteContext({ currentVoteCount: 0 }),
		);
	...
	it("should broadcast to user only when votes are hidden but voting is allowed", async () => {
		...
		vi.mocked(requireUserForApi).mockReturnValue(mockUser);
		vi.mocked(getVoteContext).mockResolvedValue(
			createMockVoteContext({ board: boardWithHiddenVotes }),
		);
		vi.mocked(castVote).mockResolvedValue({
			action: "added",
			voteId: "vote-1",
		...
		expect(response.status).toBe(200);
		expect(broadcastVoteChangedToUser).toHaveBeenCalledWith(
			"board-1",
			"card-1",
			3,
			"user-1",
