...
describe('GET /api/boards/[id]/user-votes', () => {
	...
	it('should return user voting data when user has access and votes are hidden', async () => {
		...
		const mockVotingResponse = {
			...
			user_voting_data: {
				...
				voting_allocation: { currentVotes: 2, maxVotes: 5, remainingVotes: 3, canVote: true }
			},
			voting_stats: {
				totalUsers: 5,
				activeUsers: 3,
	...
	it('should return all votes by card when showVotes is true', async () => {
		...
		const mockAllVotes = [
			{ voteId: 'v1', cardId: 'card-1', userId: 'user-1', createdAt: new Date() },
			{ voteId: 'v2', cardId: 'card-1', userId: 'user-2', createdAt: new Date() },
			{ voteId: 'v3', cardId: 'card-2', userId: 'user-3', createdAt: new Date() },
	...
	it('should handle missing currentSceneId gracefully', async () => {
		...
		const mockVotingResponse = {
			success: true,
			user_voting_data: { user_votes: [], voting_allocation: { currentVotes: 0, maxVotes: 5, remainingVotes: 5, canVote: true } },
