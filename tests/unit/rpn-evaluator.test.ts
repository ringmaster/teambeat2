...
import type { EvaluationContext, RPNExpression } from "$lib/types/scorecard";
...
describe("RPN Evaluator", () => {
	...
	describe("Complex Expressions", () => {
		...
		it("should handle threshold comparison with literals", () => {
			...
			const expr: RPNExpression = ["get", "error_rate", "literal", 0.1, "gt"];
			const result = evaluateRPN(expr, context);
			expect(result.success).toBe(true);
			expect(result.value).toBe(true);
		});
	...
	describe("Date Operations", () => {
		...
		it("should use days_since with $ shortcut", () => {
			...
			const context = [{ date: isoDate, title: "Task 1" }];
