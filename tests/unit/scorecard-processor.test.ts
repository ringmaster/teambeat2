...
describe('Scorecard Processor', () => {
  describe('Aggregate Rules (iterate_over: null)', () => {
    ...
    it('should interpolate multiple fields in templates', () => {
      ...
      expect(results.results[0].primaryValue).toBe('1500 requests (API Gateway)');
    });
  });
  ...
  describe('Detail Rules (with iteration)', () => {
    ...
    it('should include sourceData in detail results', () => {
      ...
      expect(results.results).toHaveLength(2);
      expect(results.results[0].sourceData).toEqual({ name: 'Item 1', value: 100 });
      expect(results.results[1].sourceData).toEqual({ name: 'Item 2', value: 200 });
    });
  ...
  describe('Root Array Processing (iterate_over: "$")', () => {
    it('should process root-level array with $ path', () => {
      ...
      const data = [
        { id: 'ISS-1', title: 'Security vulnerability', priority: 'high', status: 'open' },
        { id: 'ISS-2', title: 'Minor bug', priority: 'low', status: 'open' },
        { id: 'ISS-3', title: 'Performance issue', priority: 'high', status: 'in progress' },
        { id: 'ISS-4', title: 'UI tweak', priority: 'medium', status: 'closed' }
      ];
    ...
    it('should handle root array with sourceData', () => {
      ...
      expect(results).toHaveLength(2);
      expect(results[0].sourceData).toEqual({ id: 'A', status: 'active', meta: { created: '2024-01-01' } });
      expect(results[1].sourceData).toEqual({ id: 'B', status: 'pending', meta: { created: '2024-01-02' } });
    });
  ...
  describe('CSV Array Length Counting', () => {
    ...
    it('should count records with threshold-based severity', () => {
      ...
      const smallData = Array.from({ length: 50 }, (_, i) => ({ id: i, value: i * 10 }));
      const mediumData = Array.from({ length: 150 }, (_, i) => ({ id: i, value: i * 10 }));
      const largeData = Array.from({ length: 1500 }, (_, i) => ({ id: i, value: i * 10 }));
  ...
  describe('Multi-Value Stack Results', () => {
    ...
    it('should handle three values on stack', () => {
      const rule: ScorecardRule = {
        ...
        condition: ['get', 'item.name', 'get', 'item.value', 'dup', 'literal', 10, 'gt'],
