...
describe("Template Interpolator", () => {
	...
	describe("Real-World Scorecard Examples", () => {
		it("should format ticket approaching SLE", () => {
			...
			const template =
				"Approaching SLE: {ticket.id} - {ticket.title} ({ticket.days_remaining} days)";
			const result = interpolateTemplate(template, context);
			expect(result).toBe(
				"Approaching SLE: JIRA-5678 - Database Migration (2 days)",
			);
		});
		...
		it("should format deployment info", () => {
			...
			const template =
				"{deployment.service} ({deployment.version}) - Error rate: {deployment.error_rate}";
			const result = interpolateTemplate(template, context);
	...
	describe("Array Indexing", () => {
		it("should access array element by index", () => {
			...
			const template =
				"Status: {result[0]}, Days: {result[1]}, Severity: {result[2]}";
			const result = interpolateTemplate(template, context);
