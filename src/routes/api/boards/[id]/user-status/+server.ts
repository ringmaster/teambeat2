...
export const GET: RequestHandler = async (event) => {
  try {
    ...
    if (!userRole || !['admin', 'facilitator'].includes(userRole)) {
      return json(
        { success: false, error: 'Access denied. Admin or facilitator role required.' },
        { status: 403 }
      );
    }
    ...
    const userStatusList = await Promise.all(
      seriesMembers.map(async (member) => {
        ...
        const presenceInfo = presenceData.find(p => p.userId === member.userId);
    ...
    return json({
      ...
      summary: {
        ...
        totalVotesRemaining: userStatusList.filter(u => u.isActive).reduce((sum, u) => sum + u.votesRemaining, 0),
        totalVotesUsed: userStatusList.filter(u => u.isActive).reduce((sum, u) => sum + u.votesUsed, 0)
      }
    });
  } catch (error) {
