...
export const POST: RequestHandler = async (event) => {
	try {
		...
		const sourceUserRole = await getUserRoleInSeries(
			user.userId,
			sourceBoard.seriesId,
		);
		if (!sourceUserRole) {
			return json(
		...
		await withTransaction(async (tx) => {
			...
			const incompleteFreeFormAgreements =
				await findIncompleteAgreementsByBoardId(data.sourceId);
			const incompleteCommentAgreements =
				await findIncompleteCommentAgreementsByBoardId(data.sourceId);
