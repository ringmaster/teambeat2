...
import { boardSeries, boards } from "$lib/server/db/schema.js";
...
export const GET: RequestHandler = async (event) => {
	try {
		...
		const userRole = await getUserRoleInSeries(
			user.userId,
			currentBoard.seriesId,
		);
		if (!userRole) {
		...
		const otherSeriesBoards = await db
			...
			.where(
				and(
					...
					or(eq(boards.status, "completed"), eq(boards.status, "active")),
				),
			)
			.orderBy(desc(boards.createdAt));
		...
		otherSeriesBoards.forEach((board) => {
			if (board.seriesId !== currentBoard.seriesId) {
				...
				if (
					!existing ||
					new Date(board.createdAt) > new Date(existing.createdAt)
				) {
		...
		const filteredCurrentSeriesBoards = currentSeriesBoards.filter(
