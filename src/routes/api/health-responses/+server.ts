	...
	try {
		...
		if (sceneDisplayMode === "results") {
			return json(
				{
					success: false,
					error: "Survey is in results mode, responses are locked",
				},
				{ status: 403 },
			);
		}
		...
		if (
			(question.questionType === "range1to5" ||
				question.questionType === "agreetodisagree") &&
			(data.rating < 1 || data.rating > 5)
		) {
			return json(
				{
					success: false,
					error: "Range questions require rating between 1 and 5",
				},
				{ status: 400 },
			);
		}
		...
		if (
			question.questionType === "redyellowgreen" &&
			![1, 3, 5].includes(data.rating)
		) {
			return json(
				{
					success: false,
					error: "Red/Yellow/Green questions require rating of 1, 3, or 5",
				},
				{ status: 400 },
			);
		}
