	...
	try {
		...
		if (userRole !== "admin") {
			return json(
				{
					success: false,
					error: "Only series administrators can rename series",
				},
				{ status: 403 },
			);
		}
...
export const DELETE: RequestHandler = async (event) => {
	try {
		...
		console.log(
			`Attempting to delete series: ${seriesId} by user: ${user.userId}`,
		);
		...
		if (userRole !== "admin") {
			return json(
				{
					success: false,
					error: "Only series administrators can delete series",
				},
				{ status: 403 },
			);
		}
