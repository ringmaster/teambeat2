...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    const result = await collectDataAndProcess(sceneScorecardId, body.datasource_data);

    // Broadcast to board users
    broadcastScorecardDataCollected(scene.boardId, sceneScorecardId, result.processedAt, result.resultCount);
