...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    if (result.success) {
      ...
      try {
        const { buildPresentModeData } = await import('$lib/server/utils/present-mode-data.js');
        presentModeData = await buildPresentModeData(boardId, user.userId);
      } catch (error) {
        console.warn('Could not build present mode data:', error instanceof Error ? error.message : 'Unknown error');
      }
