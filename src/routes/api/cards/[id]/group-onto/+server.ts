	...
	try {
		...
		if (!getSceneCapability(currentScene, board.status, "allow_group_cards")) {
			return json(
				{
					success: false,
					error: "Grouping cards not allowed in current scene",
				},
				{ status: 403 },
			);
		}
		...
		const affectedCards = updatedCards.filter(
			(c) => c.groupId === result.targetGroupId,
		);
...
export const DELETE: RequestHandler = async (event) => {
	try {
		...
		if (originalGroupId) {
			const remainingGroupCards = updatedCards.filter(
				(c) => c.groupId === originalGroupId,
			);
