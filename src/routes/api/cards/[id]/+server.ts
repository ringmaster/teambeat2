	...
	try {
		...
		if (!getSceneCapability(currentScene, board.status, "allow_edit_cards")) {
			return json(
				{
					success: false,
					error: "Deleting cards not allowed in current scene",
				},
				{ status: 403 },
			);
		}
	...
	try {
		...
		if (
			...
			return json(
				{
					success: false,
					error: "Card sequencing not allowed in current scene",
				},
				{ status: 403 },
			);
		}
		...
		// Resequence the card
		const { affectedCardIds } = await resequenceCard(
			cardId,
			data.columnId,
			data.seq,
		);
		...
		const affectedCards = allCards.filter((c) =>
			affectedCardIds.includes(c.id),
		);
