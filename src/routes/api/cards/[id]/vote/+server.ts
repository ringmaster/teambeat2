	...
	try {
		...
		if (delta !== 1 && delta !== -1) {
			...
			return json(
				{
					success: false,
					error: "Invalid delta value. Must be 1 or -1",
					details: `Received: ${delta}`,
				},
				{ status: 400 },
			);
		}
		...
		const canShowVotes = getSceneCapability(
			board.currentScene,
			board.status,
			"show_votes",
		);
		const canAllowVoting = getSceneCapability(
			board.currentScene,
			board.status,
			"allow_voting",
		);
		...
		const voteCountsByCard = canShowVotes
			? response.all_votes_by_card || (await getVoteCountsByCard(board.id))
			: undefined;
		...
		if (canShowVotes) {
			...
			await broadcastVoteChangedToUser(
				board.id,
				cardId,
				voteCount,
				user.userId,
