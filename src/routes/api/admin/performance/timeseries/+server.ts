...
export const GET: RequestHandler = async ({ cookies, url }) => {
	...
	if (range === '1d' || range === '1w') {
		...
		const snapshots = await metricsPersistence.getHistoricalMetrics(cutoff, now);
		...
		if (filtered.concurrentUsers.length > 0 && filtered.concurrentUsers[0].timestamp > cutoff + 60000) {
