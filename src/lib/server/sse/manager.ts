...
class SSEManager {
	...
	addClient(
		...
		if (userId) {
			...
			if (userConns.size >= MAX_CONNECTIONS_PER_USER) {
				console.warn(
					`User ${userId} exceeded connection limit (${MAX_CONNECTIONS_PER_USER})`,
				);
				return false;
			}
		}
	...
	broadcastToBoard(
		...
		for (const clientId of boardClients) {
			...
			if (client) {
				...
				try {
					...
					console.error(
						"Failed to send SSE message to client",
						clientId,
						error,
					);
		...
		performanceTracker.recordBroadcast(
			duration,
			recipientCount,
			parseInt(boardId),
			message.type,
	...
	getActiveUserCount(boardId: string): number {
		return this.getBoardClients(boardId).filter(
	...
	async sendPresencePings() {
		...
		for (const boardId of boardIds) {
			try {
				...
				const nearTimeoutUserIds = new Set(
					nearTimeoutUsers.map((u) => u.userId),
				);
				...
				for (const clientId of boardClients) {
					...
					if (nearTimeoutUserIds.has(client.userId)) {
						const timeSinceLastPing = client.lastPingSent
							? now - client.lastPingSent
							: Infinity;
				...
				console.error(
					`Failed to send presence pings for board ${boardId}:`,
					err,
				);
			}
		}
