...
export async function buildPresentModeData(
	...
	if (visibleColumnIds.length > 0) {
		...
		visibleCards = (await enrichCardsWithCounts(
			visibleCards,
		)) as PresentModeCard[];
	}
	...
	if (currentScene.selectedCardId) {
		const selectedCardData = visibleCards.find(
	...
	const result = {
		...
		scene_permissions: {
			allow_comments: getSceneCapability(
				currentScene,
				board.status,
				"allow_comments",
			),
			allow_voting: getSceneCapability(
				currentScene,
				board.status,
				"allow_voting",
			),
			allow_edit_cards: getSceneCapability(
				currentScene,
				board.status,
				"allow_edit_cards",
			),
			show_votes: getSceneCapability(currentScene, board.status, "show_votes"),
			show_comments: getSceneCapability(
				currentScene,
				board.status,
				"show_comments",
			),
