...
export async function getDatabaseInfo(): Promise<DatabaseInfo> {
	if (isPostgres) {
		...
		const versionString =
			result[0]?.version || result.rows?.[0]?.version || "unknown";
		...
		const result = await sqliteDb.all<{ sqlite_version: string }>(
  ...
  `);
	...
	for (const tableRow of tablesResult) {
		...
		for (const indexRow of indexListResult) {
			...
			const indexDetails = indexDetailsResult.find(
				(idx: any) => idx.name === indexName,
			);
			const isUnique = indexDetails
				? (indexDetails as any).unique === 1
				: false;
...
export function getExpectedSchema(): { name: string; tableName: string }[] {
	...
	console.log(
		`Found ${tables.length} expected tables:`,
...
export async function checkSchemaConformance(): Promise<SchemaCheckResult> {
			...
			if (symName.includes("ExtraConfigBuilder")) {
				...
				if (typeof extraConfigBuilder === "function") {
					try {
							...
							for (const [key, value] of entries) {
								...
								if (
									...
									if (config.name) {
										...
										const indexColumns = (config.columns || [])
											.map((col: any) => {
						...
						console.log(
							`Could not call ExtraConfigBuilder for ${expectedTable.tableName}:`,
							err,
						);
					}
				}
	...
	for (const actualTable of actualTables) {
		...
		const expectedTable = expectedTables.find(
			(t) => t.tableName === actualTable.name,
		);
		...
		for (const [jsPropertyName, colDef] of Object.entries(expectedColumns)) {
			...
			if (!dbColumnName) {
				dbColumnName =
					colDefAny.name || colDefAny.columnName || colDefAny.config?.name;
			}
		...
		console.log(
			`  Expected columns:`,
			Array.from(expectedDbColumnNames).sort(),
		);
		...
		const expectedTableDef = expectedTables.find(
			(t) => t.tableName === actualTable.name,
		);
		if (expectedTableDef) {
			...
			const actualIndexNames = new Set(
				actualTable.indexes.map((idx) => idx.name),
			);
			const expectedIndexNames = new Set(
				expectedIndexes.map((idx) => idx.name),
			);
	...
	return {
		...
		issues,
		tables: {
			expected: expectedTableSummaries.sort((a, b) =>
