...
export function recordTimerVote(
	timerId: string,
	userId: string,
	choice: "A" | "B",
) {
...
export async function getBoardWithDetails(boardId: string) {
	...
	if (boardScenes.length > 0) {
		const hiddenColumns = await db
			...
			.from(scenesColumns)
			.where(and(eq(scenesColumns.state, "hidden")));
...
export async function reorderColumns(
	boardId: string,
	columnOrders: { id: string; seq: number }[],
) {
	return await withTransaction(async (tx) => {
		for (const { id, seq } of columnOrders) {
			await tx.update(columns).set({ seq }).where(eq(columns.id, id));
		}
		return { success: true };
	});
}
...
export async function getColumnsBySeriesId(
	seriesId: string,
