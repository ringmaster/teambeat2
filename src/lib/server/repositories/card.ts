...
import { cards, columns, users, votes } from "../db/schema.js";
...
export async function ungroupCard(cardId: string) {
	...
	// If no lead card exists, make the first remaining card the lead
		...
		!remainingCards.some((c) => c.isGroupLead)
	) {
		await db
			.update(cards)
			.set({
...
export async function groupCardOntoTarget(
...
export async function moveGroupToColumn(
	leadCardId: string,
	newColumnId: string,
) {
...
export async function resequenceCard(
	cardId: string,
