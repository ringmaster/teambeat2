...
import type { ProcessedResult, ScorecardRule } from "$lib/types/scorecard.js";
...
import {
	...
	processAllRules,
} from "../utils/scorecard-processor.js";

/**
 * Attach a scorecard to a scene
 */
export async function attachScorecardToScene(
	sceneId: string,
	scorecardId: string,
) {
	const id = uuidv4();
	...
	await db.insert(sceneScorecards).values({
		id,
		sceneId,
		scorecardId,
		collectedData: null,
 ...
 */
export async function getScorecardCountsByBoard(
	boardId: string,
...
export async function collectDataAndProcess(
	...
	for (const datasource of datasources) {
		...
		allResults.push(
			...results.map((r) => ({ ...r, datasourceId: datasource.id })),
		);
		allErrors.push(
			...errors.map((e) => ({ ...e, datasourceName: datasource.name })),
		);
	}
	...
	for (const result of allResults) {
		await db.insert(sceneScorecardResults).values({
			...
			secondaryValues: result.secondaryValues
				? JSON.stringify(result.secondaryValues)
				: null,
