...
import { agreements, comments, cards, columns, users, boards } from '../db/schema.js';
...
export async function updateAgreement(agreementId: string, data: UpdateAgreementData) {
...
export async function findIncompleteCommentAgreementsByBoardId(boardId: string) {
  const results = await db
    .select({
      id: comments.id,
      cardId: comments.cardId,
