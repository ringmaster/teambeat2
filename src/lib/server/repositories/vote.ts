...
export async function getVoteContext(cardId: string, userId: string) {
	...
	const [voteCountResult] = await db
		...
		.innerJoin(columns, eq(columns.id, cards.columnId))
		.where(and(eq(votes.userId, userId), eq(columns.boardId, data.boardId)));
...
export async function castVote(cardId: string, userId: string, delta: 1 | -1) {
	if (delta === 1) {
		...
		const [existingVote] = await db
			.select()
			.from(votes)
			.where(and(eq(votes.cardId, cardId), eq(votes.userId, userId)))
...
export async function getUserVotesForBoard(userId: string, boardId: string) {
	const userVotes = await db
		...
		.innerJoin(cards, eq(cards.id, votes.cardId))
		.innerJoin(columns, eq(columns.id, cards.columnId))
		.where(and(eq(votes.userId, userId), eq(columns.boardId, boardId)));
...
export async function getBoardVotingStats(boardId: string, _seriesId: string) {
	...
	const voteCountsResult = await db
		...
		.innerJoin(cards, eq(cards.id, votes.cardId))
		.innerJoin(columns, eq(columns.id, cards.columnId))
		.where(and(inArray(votes.userId, userIds), eq(columns.boardId, boardId)))
...
export async function checkVotingAllocation(
	userId: string,
	boardId: string,
...
export async function getVoteCountsByCard(
	boardId: string,
...
export async function calculateAggregateVotingStats(
	boardId: string,
	seriesId: string,
	votingAllocation: number,
	...
	return {
		...
		usersWhoHaventVoted:
			votingStats.length - votingStats.filter((u) => u.hasVoted).length,
		totalVotesCast: votingStats.reduce((sum, u) => sum + u.voteCount, 0),
		maxPossibleVotes: votingStats.length * votingAllocation,
		remainingVotes:
			votingStats.length * votingAllocation -
			votingStats.reduce((sum, u) => sum + u.voteCount, 0),
