<script lang="ts">
    ...
    rules.forEach((rule, index) => {
      ...
      if (rule.threshold_rules) {
        rule.threshold_rules.forEach((thresholdRule, thresholdIndex) => {
          ...
          const result = validateRPNCondition(
            typeof thresholdRule.condition === 'string' ? thresholdRule.condition : serializeRPNExpression(thresholdRule.condition),
            editedDatasourceSampleData,
            rule.iterate_over
          );
  ...
  async function saveScorecard() {
    ...
    try {
      saving = true;
      error = null;
      const response = await fetch(`/api/series/${seriesId}/scorecards/${selectedScorecardId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        // Update in list
        scorecards = scorecards.map(s => s.id === data.scorecard.id ? data.scorecard : s);
      } else {
  ...
  async function createDatasource() {
    ...
    try {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
      const response = await fetch(`/api/scorecards/${selectedScorecardId}/datasources`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  async function loadDatasourceDetail(datasourceId: string) {
    ...
    try {
      ...
      error = null;
      const response = await fetch(`/api/scorecards/${selectedScorecardId}/datasources/${datasourceId}`);
      const data = await response.json();
  ...
  async function saveDatasource() {
    if (!selectedDatasourceId || !selectedScorecardId || !editedDatasourceName.trim()) return;
    ...
    try {
      ...
      const parsedRules: ScorecardRule[] = rules.map(rule => {
        try {
          return {
            ...
            threshold_rules: rule.threshold_rules?.map(tr => ({
              condition: parseRPNString(typeof tr.condition === 'string' ? tr.condition : serializeRPNExpression(tr.condition)),
              severity: tr.severity
            }))
          };
        } catch (parseError) {
      ...
      const response = await fetch(`/api/scorecards/${selectedScorecardId}/datasources/${selectedDatasourceId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        selectedDatasource = data.datasource;
        // Update in list
        datasources = datasources.map(d => d.id === data.datasource.id ? data.datasource : d);
      } else {
  ...
  async function handleDatasourceDrop(e: DragEvent, dropIndex: number) {
    e.preventDefault();
    if (!selectedScorecardId || draggedDatasourceIndex === null || draggedDatasourceIndex === dropIndex) {
    ...
    try {
      const response = await fetch(`/api/scorecards/${selectedScorecardId}/datasources/reorder`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  function removeThresholdRule(ruleIndex: number, thresholdIndex: number) {
    ...
    rule.threshold_rules = rule.threshold_rules.filter((_, i) => i !== thresholdIndex);
    rules = [...rules]; // Trigger reactivity
  ...
  function moveThresholdRuleDown(ruleIndex: number, thresholdIndex: number) {
    const rule = rules[ruleIndex];
    if (!rule.threshold_rules || thresholdIndex === rule.threshold_rules.length - 1) return;
      ...
      {:else}
        <div class="list">
          {#each scorecards as scorecard}
            <div
              class="list-item"
              class:selected={selectedScorecardId === scorecard.id}
              onclick={() => selectScorecard(scorecard.id)}
            >
              <div class="list-item-content">
                <div class="list-item-title">{scorecard.name}</div>
                {#if scorecard.description}
                  <div class="list-item-description">{scorecard.description}</div>
                {/if}
              </div>
              {#if canEdit}
                <button
                  class="list-item-delete"
                  onclick={(e) => deleteScorecard(scorecard.id, e)}
            ...
            <div class="list">
              {#each datasources as datasource, index}
                <div
                  ...
                  draggable={canEdit}
                  ondragstart={() => handleDatasourceDragStart(index)}
                  ondragover={handleDatasourceDragOver}
                  ondrop={(e) => handleDatasourceDrop(e, index)}
                ...
                >
                  ...
                  {/if}
                  <div class="list-item-content">
                    <div class="list-item-title">{datasource.name}</div>
                    <div class="list-item-meta">
                      {datasource.sourceType === 'paste' ? 'Manual Paste' : 'API'} • {JSON.parse(datasource.rules).length} rule(s)
                    </div>
                  </div>
                  {#if canEdit}
                    <button
                      class="list-item-delete"
                      onclick={(e) => deleteDatasource(datasource.id, e)}
                  ...
                  {#if editingRuleIndex === index}
                    <div class="rule-editor">
                      <div class="form-group">
                        ...
                        <select bind:value={rule.iterate_over} onchange={saveDatasource} disabled={!canEdit}>
                      ...
                      <div class="form-group">
                        <label>Section</label>
                        <input type="text" bind:value={rule.section} onblur={saveDatasource} disabled={!canEdit} placeholder="e.g., Section A" />
                        <small>Section name for grouping results. Supports templates like <code>{'{$.section}'}</code></small>
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <select bind:value={rule.severity} onchange={saveDatasource} disabled={!canEdit}>
                          <option value="info">Info</option>
                          <option value="warning">Warning</option>
                          <option value="critical">Critical</option>
                        </select>
                        <small>Default severity when no threshold rules match</small>
                      </div>

                      <div class="form-group">
                        <label>Threshold Rules (optional)</label>
                        <small>Override severity based on additional conditions</small>
                        ...
                        {#if rule.threshold_rules && rule.threshold_rules.length > 0}
                          <div class="threshold-rules-list">
                            {#each rule.threshold_rules as thresholdRule, thresholdIndex}
                              <div class="threshold-rule-item">
                                <div class="threshold-rule-header">
                                  <span class="threshold-rule-number">Threshold {thresholdIndex + 1}</span>
                                  {#if canEdit}
                                    <div class="threshold-rule-controls">
                                      <button
                                        onclick={() => moveThresholdRuleUp(index, thresholdIndex)}
                                      ...
                                      <button
                                        onclick={() => moveThresholdRuleDown(index, thresholdIndex)}
                                        disabled={thresholdIndex === rule.threshold_rules!.length - 1}
                                        title="Move down"
                                        class="btn-control"
                                      >
                                        ↓
                                      ...
                                      <button
                                        onclick={() => removeThresholdRule(index, thresholdIndex)}
                                ...
                                <div class="threshold-rule-fields">
                                  <div class="form-group-inline">
                                    <label>Condition</label>
                                    <input
                                      type="text"
                                      bind:value={thresholdRule.condition}
                                      onblur={saveDatasource}
                                      placeholder="e.g., get_json_value count 20 gt"
                                      disabled={!canEdit}
                                      class:validation-error={thresholdRuleValidations.get(`${index}-${thresholdIndex}`) && !thresholdRuleValidations.get(`${index}-${thresholdIndex}`)!.valid}
                                    />
                                  </div>
                                  {#if thresholdRuleValidations.get(`${index}-${thresholdIndex}`)}
                                    {@const validation = thresholdRuleValidations.get(`${index}-${thresholdIndex}`)!}
                                    {#if !validation.valid}
                                      <div class="validation-message error">
                                        {validation.error}
                                        {#if validation.details}
                                  ...
                                  <div class="form-group-inline">
                                    ...
                                    <select bind:value={thresholdRule.severity} onchange={saveDatasource} disabled={!canEdit}>
                                      <option value="info">Info</option>
                                      <option value="warning">Warning</option>
                                      <option value="critical">Critical</option>
                                    </select>
                        ...
                        {#if canEdit}
                          <button onclick={() => addThresholdRule(index)} class="btn-secondary btn-small">
                            + Add Threshold Rule
                          </button>
                        {/if}
                      </div>
