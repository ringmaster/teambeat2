...
export function buildDisplayRuleContext(
	...
	const scorecardCount =
		scorecardCountsByScene && scene?.id
			? scorecardCountsByScene[scene.id] || 0
			: 0;
...
export function evaluateDisplayRule(
	...
	try {
		...
		const context = buildDisplayRuleContext(
			board,
			scene,
			cards,
			agreements,
		...
		if (!result.success) {
			console.error(
				`Display rule evaluation failed for scene "${scene.title}":`,
				result.error,
			);
		...
		console.error(
			`Error evaluating display rule for scene "${scene.title}":`,
			error,
		);
		// On error, default to showing the scene
		return true;
