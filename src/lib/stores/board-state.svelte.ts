...
export function createBoardStore() {
  ...
  return {
    ...
    // Scene operations
    addScene(scene: any) {
      console.log("[Store] addScene called with:", scene);
      if (board) {
        const oldScenesCount = board.scenes?.length || 0;
        board = {
          ...board,
          scenes: [...(board.scenes || []), scene],
        };
        console.log(
          "[Store] scenes updated:",
          oldScenesCount,
          "->",
          board.scenes.length,
        );
      } else {
        console.warn("[Store] addScene called but board is null");
      }
    },
