...
export class BoardSSEService {
  ...
  private handleMessage(data: any) {
    ...
    switch (data.type) {
      ...
      case "scene_changed":
        if (store.board && data.scene) {
          ...
          if (store.board.scenes) {
            const sceneIndex = store.board.scenes.findIndex(
          ...
          if (data.scene.mode === "present") {
            if (data.present_mode_data) {
              ...
              if (data.present_mode_data.selected_card) {
                store.updateScene(data.scene.id, {
                  selectedCardId: data.present_mode_data.selected_card.id,
                });
              }
            } else {
      ...
      case "scene_created":
        console.log("[SSE] scene_created received:", data.scene);
        if (data.scene) {
          store.addScene(data.scene);
          console.log("[SSE] scene added to store, scenes count:", store.board?.scenes?.length);
        } else {
          console.warn("[SSE] scene_created message missing scene data");
        }
        break;
      ...
      case "update_presentation":
        if (store.currentScene?.mode === "present") {
          ...
          if (data.card_id !== undefined) {
            store.updateScene(store.currentScene.id, {
              selectedCardId: data.card_id,
            });
          }
          ...
          if (data.present_mode_data) {
            ...
            if (data.present_mode_data.comments) {
              ...
              const commentsChanged =
                store.comments.length !== newComments.length ||
                !store.comments.every((c, i) => c.id === newComments[i]?.id);
              if (commentsChanged) {
            ...
            if (data.present_mode_data.agreements) {
              ...
              const agreementsChanged =
                store.agreements.length !== newAgreements.length ||
                !store.agreements.every(
      ...
      case "voting_stats_updated":
        ...
        if (data.votes_cleared) {
          ...
          store.clearAllVotes();
          const resetCards = store.cards.map((card) => ({
            ...card,
            voteCount: 0,
          }));
      ...
      case "scene_updated":
        if (
          data.scene_id &&
          store.currentScene?.id === data.scene_id &&
          store.currentScene?.mode === "survey"
        ) {
          window.dispatchEvent(
 ...
 */
export function createBoardSSEService(
  config: BoardSSEServiceConfig,
): BoardSSEService {
